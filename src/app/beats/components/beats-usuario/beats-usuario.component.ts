...
@Component({
  selector: 'beats-usuario',
  templateUrl: './beats-usuario.component.html',
  providers: [UserService, ListaService, BeatService, LikeBeatService, FollowService]
})

export class BeatsUsuarioComponent implements OnInit {
	...
	public viendoTab = "bases";

	public editandoMasInfo = false;
	public editandoComprarBeats = false;
	...
	abrirCerrarFormComprarBeats(accion){
		this.editandoComprarBeats = true;
		if(accion == 'abrir') this.editandoComprarBeats = true;
		if(accion == 'cerrar') this.editandoComprarBeats = false;
	}

	abrirCerrarFormMasInfo(accion){
		if(accion == 'abrir') this.editandoMasInfo = true;
		if(accion == 'cerrar') this.editandoMasInfo = false;
	}
		...
		this._userService.updateUser(this.user).subscribe(
			response => {
				if(!response.user){
					...
					this.abrirCerrarFormComprarBeats('cerrar');
					this.abrirCerrarFormMasInfo('cerrar');
				}
	...
	refresh(event = null){
	 ...
	 // LISTAS DEL PRODUCTOR

	 getListas(nick){
		this._listaService.getListasUsuario(nick).subscribe(
			response => {
				if(response.listas){
