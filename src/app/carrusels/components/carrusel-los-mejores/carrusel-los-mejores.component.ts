...
@Component({
  selector: 'carrusel-los-mejores',
  templateUrl: './carrusel-los-mejores.component.html',
  styleUrls: ['./carrusel-los-mejores.component.scss'],
  providers: [UserService, DestacadoService]
})
export class CarruselLosMejoresComponent implements OnInit {
	public url: string;
	...
	public mejorEnFreestyle: User;
	public mejorEnBatallas: User;
	public mejorEnBeats: User;
	...
	ngOnInit(){
		...
		this.getMejoresUsuarios();
	}
	
	getMejoresUsuarios(){
		this._userService.getMejoresUsuarios().subscribe(
			response => {
				if(response){
					this.mejorEnBatallas = response.mejorEnBatallas;
					this.mejorEnFreestyle = response.mejorEnFreestyle;
					this.mejorEnBeats = response.mejorEnBeats;
				} else {
					this.status = 'error'
				}
			},
