...
@Component({
  selector: 'carrusel-batallas',
  templateUrl: './carrusel-batallas.component.html',
  providers: [UserService, DestacadoService]
})
export class CarruselBatallasComponent implements OnInit {
	public url: string;
	...
	ngOnInit(){
		console.log('Carrusel de batallas destacadas cargado');
    this.getDestacados("batalla");
  }
    ...
    this._destacadoService.getDestacados(tipo).subscribe(
      response => {
        if(response.destacados){
          ...
          console.log("Destacados batalla: ", this.destacados);
