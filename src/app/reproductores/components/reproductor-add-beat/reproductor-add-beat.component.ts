...
@Component({
  selector: 'reproductor-add-beat',
  templateUrl: './reproductor-add-beat.component.html',
  providers: [UserService, BeatService, FollowService, ListaService]
})

export class ReproductorAddBeatComponent implements OnInit, OnChanges {
    // Decorador Output
    ...
    ngOnChanges(changes: SimpleChanges) {
            ...
            if(beat){
                ...
                if(beat.currentValue){
                    ...
                    this.getBeatsUsuario(this.identity.nick);
	...
	recibirTipoCaratula(event):void{
		this.PasarTipoCaratula.emit({tipoCaratula: event.tipoCaratula});
	}

	recibirAjustesCaratula(event):void{
		this.PasarAjustesCaratula.emit({ajustesCaratula: event.ajustesCaratula});
	}
	...
	comprobarSeguimientoUsuario(){
		...
		if(this.identity && this.idsUsuariosQueSigo && this.idsUsuariosQueSigo.indexOf(this.identity._id) >= 0){
			this.siguiendo = true;
		} else if(this.identity && this.idsUsuariosQueSigo && this.idsUsuariosQueSigo.indexOf(this.identity._id) < 0){
