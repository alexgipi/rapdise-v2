...
@Component({
  selector: 'modal-responder-batalla-en-proceso',
  templateUrl: './modal-responder-batalla-en-proceso.component.html',
  providers: [UserService, BatallaService,UploadService, NotificacionService, PalabrasService]
})

export class ModalResponderBatallaEnProcesoComponent implements OnChanges, OnInit {
	...
	public tocaGrabarImpro = 4;
	public usuarioQueToca;
	...
	ngOnChanges(changes: SimpleChanges) {
		...
		this.getBatalla(this.batallaSeleccionada);
	...
	getBatalla(batalla){
		this.palabrasGrabacion = batalla.palabrasImpro1.split(',');
		console.log(this.palabrasGrabacion);

		this.beat2 = batalla.base2;
		this.beat3 = batalla.base3;


		if(batalla.impro2 != null && batalla.impro3 == null){
			this.tocaGrabarImpro = 3;
			this.beatSeleccionado = this.beat2;
		} else if(batalla.impro3 != null && batalla.impro4 == null){
			this.tocaGrabarImpro = 4;
			this.beatSeleccionado = this.beat2;
		} else if(batalla.impro4 != null && batalla.impro5 == null){
			this.tocaGrabarImpro = 5;
			this.beatSeleccionado = this.beat3;
		} else if(batalla.impro5 != null && batalla.impro6 == null){
			this.tocaGrabarImpro = 6;
			this.beatSeleccionado = this.beat3;
		}

		if(this.tocaGrabarImpro == 4 || this.tocaGrabarImpro == 5){
			this.usuarioQueToca = batalla.usuario1;
		} else if(this.tocaGrabarImpro == 3 || this.tocaGrabarImpro == 6){
			this.usuarioQueToca = batalla.usuario2;
		}

		console.log('Toca grabar impro: '+this.tocaGrabarImpro);
		console.log('Le toca al usuario: ' + this.usuarioQueToca.nick);
	}
	...
	startPlayer(beat){
  	...
  	empezarGrabacion() {
		...
		setTimeout( function(){
			that.startPlayer(that.beatSeleccionado);
		...
		const onSuccess = stream => {
			...
			this.mediaRecorder.onstop = e => {
				...
				blobToBase64(blob, function(base64){ // encode
					...
					if(that.batallaSeleccionada.rondas == 2){
						if(that.tocaGrabarImpro == 4){
							if(that.videoActivado == true) that.batallaSeleccionada.impro4 = that.audioBatalla.nombreArchivo+".webm";
							if(that.videoActivado == false) that.batallaSeleccionada.impro4 = that.audioBatalla.nombreArchivo+".wav";
							that.batallaSeleccionada.palabrasImpro4 = that.palabrasBatalla.join();
							that.batallaSeleccionada.estado = "Finalizada";
						} 
					} else if(that.batallaSeleccionada.rondas == 3){
						if(that.tocaGrabarImpro == 4){
							if(that.videoActivado == true) that.batallaSeleccionada.impro4 = that.audioBatalla.nombreArchivo+".webm";
							if(that.videoActivado == false) that.batallaSeleccionada.impro4 = that.audioBatalla.nombreArchivo+".wav";
							that.batallaSeleccionada.palabrasImpro4 = that.palabrasBatalla.join();
							that.batallaSeleccionada.estado = "En marcha";
						} else if(that.tocaGrabarImpro == 5){
							if(that.videoActivado == true) that.batallaSeleccionada.impro5 = that.audioBatalla.nombreArchivo+".webm";
							if(that.videoActivado == false) that.batallaSeleccionada.impro5 = that.audioBatalla.nombreArchivo+".wav";
							that.batallaSeleccionada.palabrasImpro5 = that.palabrasBatalla.join();
							that.batallaSeleccionada.estado = "En marcha";
						} else if(that.tocaGrabarImpro == 6){
							if(that.videoActivado == true) that.batallaSeleccionada.impro6 = that.audioBatalla.nombreArchivo+".webm";
							if(that.videoActivado == false) that.batallaSeleccionada.impro6 = that.audioBatalla.nombreArchivo+".wav";
							that.batallaSeleccionada.palabrasImpro6 = that.palabrasBatalla.join();
							that.batallaSeleccionada.estado = "Finalizada";
						} 
					}
	...
	modificarBatalla(batalla){
		...
		response => {
			if(response.batalla){
				...
				if(this.batallaSeleccionada.rondas == 2){
					alert("2 rondas")
				...
				if(this.batallaSeleccionada.rondas == 3){
					alert("3 rondas")
					
					 if(this.batallaSeleccionada.impro4 != null && this.batallaSeleccionada.impro5 == null){
						this.tocaGrabarImpro = 5;
	
					} else if(this.batallaSeleccionada.impro5 != null && this.batallaSeleccionada.impro6 == null){
						this.tocaGrabarImpro = 6;
						this.crearNotificacion(batalla._id,batalla.usuario2._id,this.identity.nick + " te ha contestado");

					} else if(this.batallaSeleccionada.impro6 != null){
						this.tocaGrabarImpro = null;
						this.crearNotificacion(batalla._id,batalla.usuario1._id,"La batalla " + batalla.nombre + " ha finalizado!");
						this.crearNotificacion(batalla._id,batalla.usuario2._id,"La batalla " + batalla.nombre + " ha finalizado!");
