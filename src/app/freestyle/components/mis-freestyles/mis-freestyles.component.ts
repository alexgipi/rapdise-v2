...
@Component({
  selector: 'mis-freestyles',
  templateUrl: './mis-freestyles.component.html',
  providers: [UserService, GrabacionService, BeatService]
})
export class MisFreestylesComponent implements OnInit {
  ...
  constructor(
    ...
    ngOnInit(){
        ...
        this.getGrabacionesUsuario(this.identity.nick);
           
    }

    getGrabacionesUsuario(nick){
      this._grabacionService.getGrabacionesUsuario(nick).subscribe(
        response => {
